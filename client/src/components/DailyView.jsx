...
export default function DailyView(props) {
    ...
    useEffect(() => {
        if (propsIsArray(props.tasks)) {
            ...
            const tasks_with_duedate_today = tasks_with_localized_dateTime.filter((taskItem) => {
                return taskItem.due_date.includes(getConventionalDateFormat(getMinDate()))
            })

            setTodaysTasks(tasks_with_duedate_today)
        ...
        <div className='dailyView-task-area'>
            ...
            <section className='dailyView-tableContainer'>
                <table class="table dailyView-tableStyle">
                    ...
                    <tbody>
                        {!showCompleted && propsIsArray(todaysTasks) && todaysTasks.map(taskItem => {
                            return <ListItem 
